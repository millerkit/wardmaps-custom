...
function tryGettingPlaceFromQuery(placesService, addressFieldValue, PlacesServiceStatus) {
...
async function initAutocomplete() {
	...
	searchForm.onsubmit = async () => {
		const addressFieldValue = addressField.value ? addressField.value.trim() : null
			...
			autocompleteService.getPlacePredictions(
					...
					async (results, status) => {
						if (status === PlacesServiceStatus.OK && results.length) {
							...
							await getPlaceDetails(placesService, results[0].place_id, PlacesServiceStatus)
						}
					}
			)
...
function getAddressComponents(newPlace) {
	return newPlace.address_components.reduce((acc, curr) =>
			(acc[curr.types[0]] = {
...
async function handlePlaceChange(newPlace) {
	...
	if (newPlace.formatted_address && newPlace.geometry && newPlace.address_components) {
